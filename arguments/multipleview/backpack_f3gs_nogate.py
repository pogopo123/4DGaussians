"""F3GS ablation -- fusion only, no gate.

Strips the motion mask head and the velocity head. The Flow-HexPlane keeps its
one remaining job: supplying Key/Value to the cross-attention, so the velocity
feature still reaches the deformation heads. Nothing multiplies the deltas.

What this isolates: the baseline run showed the gate cutting 71.6% of the
displacement on the very points it called dynamic, and reproducing only 53.5% of
the real motion. Without the gate that throttle is gone, so this run answers
"how much of the motion gap was the gate, and how much was the deformation field
itself?" -- at the cost of the early-exit pass, which has nothing to threshold.
...
ModelHiddenParams = dict(
    ...
    flow_net_width = 64,
    flow_net_depth = 2,
    flow_velocity_head = False,   # ablation: removed
    flow_mask_init_bias = 2.0,    # unused when flow_gate=False
    flow_gate = False,            # ablation: no mask head, deltas ungated
    ...
    early_exit = False,            # unavailable without a mask
    ...
    lambda_flow = 10.0,
    lambda_flow_velocity = 0.0,   # velocity head is gone
    ...
    lambda_motion_sparse = 0.0,   # no m to sparsify
    lambda_motion_bind = 0.0,  # superseded by L_flow -- set >0 only for the ablation
    lambda_motion_smooth = 0.0,   # no m to smooth

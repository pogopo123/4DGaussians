"""F3GS-split-hires -- split nhưng Flow-HexPlane có đủ capacity.

Run split đầu tiên đạt PSNR 32.42 (cao nhất) nhưng chuyển động chỉ 38.6% (kém
nogate 19 điểm). Nguyên nhân: dx và dq chuyển từ đường App-HexPlane (297.4M tham
số, phân giải tới 2048) sang Flow-HexPlane (3.7M, tới 256) -- nhỏ hơn 80 lần và
thô hơn 8 lần. Cấu hình phân giải thấp đó được đặt hồi nhánh flow chỉ sinh MASK;
một mask nhị phân mềm thì phân giải thấp là prior tốt, một trường DỊCH CHUYỂN 3D
thì không.

Run này cho Flow-HexPlane đúng bằng App-HexPlane để tách biến capacity ra khỏi
giả thuyết cần kiểm (cách chia head có đúng không).

Header gốc:

F3GS-split -- mỗi nhánh một loại thuộc tính.

Flow-HexPlane -> dx, dq   (SE(3): tịnh tiến + xoay = chuyển động cứng)
App-HexPlane  -> ds, dopacity, dSH   (bức xạ + hình dạng nội tại)

Không có cross-attention, không có backbone chung, không có residual chung -- mọi
đường nối giữa hai nhánh đều đi qua sg(). Hệ quả đo được: L_flow chỉ còn chạm
Flow-HexPlane (kiến trúc cũ để nó định hình App-HexPlane tới 49.1%).

L_rgb vẫn tới được dx: flow prior chỉ phủ 1.76% pixel có chuyển động thật, nên
chặn nó đi sẽ để chuyển động không ràng buộc ở mọi vùng flow thiếu. Bật
flow_split_detach_rgb=True nếu muốn cô lập tuyệt đối.

Header gốc:

F3GS -- Flow-Guided 4D Gaussian Splatting.

Dual-Grid HexPlane (appearance + velocity) merged by Cross-Attention, decoded by
a shared backbone into a deformation head and a motion-mask head, gated by
Delta = m_i * delta, and supervised by a real optical-flow prior.

Both grids are queried directly at (x, y, z, t) -- no Fourier encoding of t.
"""
